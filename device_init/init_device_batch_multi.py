...
from device_init.init_device_batch import InitDeviceBatch
import pandas as pd

"""
    批初始化多个服务器上的设备
"""
class InitDeviceBatchMulti(RequestOperationBatchMulti):
    ...
    def _make_params(self, df: pd.DataFrame):
        ...
        for index, row in df.iterrows():
            ...
            model = row['Model'].lower()                                # 型号如SRX-R320等, 需要转小写
            ip = row['Management IP Address']                           # IP直接用
            aud_num = str(row['Auditorium Number'])
            port = str(row['port'])
