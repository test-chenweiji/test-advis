...
class InitDeviceBatch(RequestOperationBatch):
    ...
    def pre_request(self):
        ...
        for item in self._data_tuple:
            ...
            try:
                if DBQuery.is_exist(
                        self._url,
                        'Device', "screen_uuid = '{}' and type = '{}' and category = '{}' and model = '{}'"
                                .format(screen_uuid, manufacturer, category, model), cookies=cookies):
                    print('影厅编号:{}, uuid:{}, 设备:{} 已存在,不作新增'.format(aud_num, screen_uuid, category))
                    continue
            except DBQueryException as e:
                print('影厅编号:{}, uuid:{}, 设备:{}, 发生DBQueryException:{}, 不作新增'.format(aud_num, screen_uuid, category, e))
                continue
            except Exception as e:
                print('数据库查询出现异常:{}，忽略此次批处理'.format(e))

            data = item['device']
            data['screen_uuid'] = screen_uuid
            data['slave']['screen_uuid'] = screen_uuid
